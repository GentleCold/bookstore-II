...
class TestPick:
    ...
    def pre_run_initialization(self):
        self.seller_id = "test_pick_seller_id_{}".format(str(uuid.uuid1()))
        self.store_id = "test_pick_store_id_{}".format(str(uuid.uuid1()))
        self.buyer_id = "test_pick_buyer_id_{}".format(str(uuid.uuid1()))
    ...
    def test_not_valid_order(self):
        code = self.buyer.pick(self.order_id + "_x")
        assert code == 518

    def test_repeat_pick(self):
        code = self.buyer.pick(self.order_id)
        assert code == 200
        code = self.buyer.pick(self.order_id)
        assert code == 523
